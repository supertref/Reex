...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a REL address (e.g. %1)").arg("s7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no reliance: URI
...
{
    // Convert reliance:// to reliance:
    //
    //    Cannot handle this later, because reliance:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open reliance.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Reliance.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Reliance.lnk
...
{
    return GetAutostartDir() / "reliance.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a reliance.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Reliance\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the reliance app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add reliance app to startup item list
