...
#if defined(NDEBUG)
#error "unnycore cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uunny) are considered zero fee (for relaying and mining)
...
{
    RenameThread("unnycore-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // UNNY: We use certain sporks during IBD, so check to see if they are
